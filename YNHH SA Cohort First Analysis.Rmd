...
all_pts <- all_pts %>% mutate(
  ...
  sane_kit_YN = case_when(
    ...
    sane_kit_done == 4 ~ "Note done due to concurrent psychiatric concerns"
  ),
...
excluded_patients <- excluded_patients %>% add_value_labels(reason_to_exclude = c( "Seen earlier" = 1 , "Patient reports not being assaulted" = 2, "Psych" = 3, "not excluded" = 3, "Eloped" = 5)) %>% to_factor()
...
```{r}
table.1 <- cohort.2 %>% 
  select(pathway, age, age_3_group, female, race_bwo, trauma_case_yn, contacted_advocate, sane_kit_YN, sane_kit_missed) %>% 

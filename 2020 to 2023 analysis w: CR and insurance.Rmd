...
variables2 <-
  DF2 %>% select(gender,arrive_dt:primary_coverage_benefit_plan_name.y) %>% colnames()
variables3 <-
...
r_DF1 <- DF2 %>%
  ...
  mutate(
    ...
    ) #only 5 patients have NA for patient.language.x. Will drop patient_language.y
  ) 
```
...
```{r}
full_merge_DF <- left_join(r_DF2, short_trauma_cat, by = join_by(pat_enc_csn_id)) %>% distinct() %>% select(-patient_language.y) %>% rename(patient_language = patient_language.x, mrn = mrn.x, patient_birth_date = patient_birth_date.x)

```
now to reduce the # of variables from 183 to something more managable
```{r}
final_variables <-full_merge_DF %>% select(1:2,15:89,103:125,139:143, 157:158,174:183) %>% select(-ends_with(".y")) %>%  colnames()
#final_variables <- c("pat_enc_csn_id", "ed_arrival_date","age", "age_group", "age_3_group", "patient_language", "mrn.x", "patient_birth_date.x", "bmi", "weight_kg", "height_m", "documentation_of_interpreter", "time_documented", "patient_advocate_called", "patient_advocate_present", "sane_kit_done", "role_of_person_doing_the_kit", "relationship_with_patient", "sex_of_assailant", "ct_400_forensic_toxiclogy", "method_of_arrival", "follow_up_at_discharge", "if_yes_whom" )
...
```{r}
write_csv(full_merge_reduced, "SA_pts_reformatted")
```

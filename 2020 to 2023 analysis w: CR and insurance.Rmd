...
```{r}
full_merge_DF <- left_join(r_DF2, short_trauma_cat, by = join_by(pat_enc_csn_id)) %>% distinct() %>% select(-patient_language.y) %>% rename(patient_language = patient_language.x, mrn = mrn.x)

...
# single_CSN <- C_F_Trauma %>% group_by(pat_enc_csn_id) %>% mutate(dx_code=row_number()) %>% ungroup() %>% 
#   pivot_wider(names_from = dx_code, values_from = c(icd10_dx_code, dx_name), values_fill = NA) %>% group_by(pat_enc_csn_id, mrn, esi_level, trauma_case_yn)
# C_F_Trauma %>% group_by(pat_enc_csn_id) %>% count() %>% arrange(desc(n))
# new problem... one patient has 720 different dx, another has 220
...
majory_injury <- ICD_names %>% filter(category == "Minor Injury") %>% pull(var = dx, name = category)
...
C_F_Trauma_dxcat <- C_F_Trauma %>% 
  mutate(
    ...
    Majory_injury = if_else(dx_name %in% majory_injury, 1, NA),
...
```{r}
etohers <- C_F_Trauma_dxcat %>% select(pat_enc_csn_id, Etoh) %>% filter(!is.na(Etoh)) %>% distinct()
a <- left_join(short_trauma, etohers, by = "pat_enc_csn_id")
painers <- C_F_Trauma_dxcat %>% select(pat_enc_csn_id, Pain) %>% filter(!is.na(Pain)) %>% distinct()
# %>% group_by(pat_enc_csn_id) %>% add_count() %>% ungroup() %>% arrange(desc(n))
```


list of types of imaging patients had:
```{r}
C_F_Trauma %>% distinct(proc_display_name) %>% print ()
only_pts_we_care_about <- C_F_Trauma %>% filter(C_F_Trauma$pat_enc_csn_id %in% r_DF2$pat_enc_csn_id)
```

...
table.1 <- cohort.2 %>% 
  select(pathway, age, female, race_bwo, under120h, english, ambulance, insurance, relationship_with_patient) %>% 
...
```
```{r echo=FALSE, message=FALSE, warning=FALSE}
table.2 <- cohort.2 %>% select(pathway, contacted_advocate, abx_given, sti_tx, hiv_pep_kit_ordered_num, hiv_labs, female_u55, pregnancy_test_ordered_num, prevent.preg, sane_kit_YN, sane_kit_missed, follow_up_at_discharge) %>% 
...
Control for age, race (black, white, other), sex, patient language, Relationship with Patient, ICD10 with psych, ICD10 with etch, ICD10 with substance use, Insurance
Things I care less about: ICD10 with any traumatic injury, ICD10 with major trauma, ICD10 with minor trauma, trauma consult, Method of Arrival, ESI, 
```{r}
...
model.secondary.1 <-  glm(sane_kit_offered ~ pathway + age + female + race_bwo + insurance_3 + english + relationship_4_with_pt,  family = "binomial", data = cohort.3)

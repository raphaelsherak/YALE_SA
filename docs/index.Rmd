...
### Restricted to YSC, SRC, Shoreline

```{r}
#create cohort restricted to YSC, SRC, Shoreline (SAFE program sites)
SAFE_Sites <- list("YSC", "SRC", "Shoreline") #note variable sane_ED was classfied as DCed from SRC/YSC/ or YSC CIU...

SAFE_Site_Cohort <- pp.cohort.1 %>% 
  filter(arrival_location %in% SAFE_Sites)
...
tbl.reg.ss.sk.did <- sane.did.reg(SAFE_Site_Cohort_u120, SAFE_Site_Cohort_u120$exposure) %>% as_gt() %>% tab_header("Odds of Doing SAFE Kit")
tbl.reg.ss.sk.did

# advocate offered

tbl.reg.ss.adv <- advocate.regression(SAFE_Site_Cohort, SAFE_Site_Cohort$exposure) %>% as_gt() %>% tab_header("Odds of Offering Advocate")
tbl.reg.ss.adv
```

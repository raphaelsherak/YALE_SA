...
note: restricted to pts \<120 hours from assault, no longer restricted to female (only 12 men) english speakers (only 11 non-english speaking)

```{r sane kit fxns}
sane.off.reg<-function(data, exposure){
  model <-  logistf(sane_kit_offered ~ exposure + age + race_eth_num + ipv+ intoxicated + sane_ED, family = "binomial", data = data, flic = TRUE)
  model.results <- tbl_regression(model, exponentiate = TRUE) %>% add_global_p()
...
Method: *2-Profile penalized log-likelihood*

SANE Kit Offered?

```{r Sane kit offered regression, message=FALSE, warning=FALSE}
...
note: race causes quasi seperation. Emailing Jim Dzuria to get his imput about best next steps.

```{r follow up regression function, include=FALSE}
...
OR_Data <- OR_Data %>%  mutate(Measure = fct_relevel(Measure, "Antibiotics Ordered", "HIV Prophylaxis Ordered", "Pregnancy Prophylaxis Ordered", "Pregnancy Test Ordered","SANE KIT Done", "SANE Kit Offerred" ,"Advocate Offerred" )) 
...
pd <- position_dodge(width = 0.5)
  ...
  CI_Log_Graph <- p + geom_point(position = pd,aes(shape=Exposure, color=Exposure)) + geom_errorbarh(aes(xmax = UCI, xmin = LCI, color = Exposure),height = 0.1,position = pd) + labs(x =FinalArrowMsg, y = "ED Outcome Measures",color = "Exposure", shape= "Exposure")+
    theme(
...
month_line_df <-
  ...
  ) %>% gather(key = "Metric", value = "Percent", -month) %>% filter(Percent !=0) %>% mutate(Metric = factor(Metric, labels = c("Percent_Advocate" = "% Advocate",  "Percent_Pathway" = "% Pathway", "Percent_Sane" = "% SANE")))
...
quarter_line_df <-
  ...
  group_by(quarter) %>% summarise(
    ...
    Percent_Sane = mean(did_SANE_kit, na.rm = TRUE)
  ) %>% gather(key = "Metric", value = "Percent", -quarter) %>% filter(Percent !=0) %>% mutate(Metric = factor(Metric, labels = c("Percent_Advocate" = "% Advocate",  "Percent_Pathway" = "% Pathway", "Percent_Sane" = "% SANE")))
...
CI_Plot + theme_light() + guides(color = F) + coord_flip()
...
lnCI_Plot + guides(color = F) + theme_light() + theme(axis.text.x = element_text(angle = 90), axis.title.x = element_blank()) + coord_flip()
```

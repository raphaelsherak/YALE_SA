...
model.primary <-  glm(advocate_offered ~ pathway + age + female + race_bwo + insurance_3 + english,  family = "binomial", data = cohort.2)
...
model.primary.2 <-  glm(advocate_offered ~ pathway + age + female + race_bwo + insurance_3 + english,  family = "binomial", data = cohort.2.2)
...
pp.model.primary <-  glm(advocate_offered ~ exposure.num + age + female + race_bwo + insurance_3 + english + relationship_4_with_pt,  family = "binomial", data = pp.cohort.2)
...
pp.model.primary.2 <-  glm(advocate_offered ~ exposure.num + age + female + race_bwo + insurance_3 + english,  family = "binomial", data = pp.cohort.2.2)
...
```{r}
d.n.cohort <- all_pts %>% filter(age > 17) %>% filter(is.na(exclude))
supp.table.1 <- d.n.cohort %>% 
  select(day_night, age, female, race_bwo, under120h, english, ambulance, insurance, relationship_with_patient, advocate_offered, sane_kit_YN) %>% 

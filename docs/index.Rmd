...
x<- data %>% mutate(sane_kit_u120 = case_when(
  ...
  set_variable_labels(
    ...
    follow_up_at_discharge = "Has after discharge follow up"
                    ...
                    missing = "no",
                    statistic = list(
                      c(hiv_pep_kit_ordered_num, prevent.preg, abx_given, pregnancy_test_ordered_num, sane_kit_u120) ~ "{n}"
                    ))
return(x)
}
...
tbl.pep.hiv <- tbl_merge(list(tbl.pep.pp, tbl.pep.path), tab_spanner = c("**Pre v. Post**", "**Pathway v. No Pathway**")) %>% as_gt() %>% tab_header("Odds of Receiving HIV PEP Presented to ED <72 Hours Since Assault")
tbl.pep.hiv
```

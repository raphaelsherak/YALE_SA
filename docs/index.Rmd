...
epicdata <- read_excel("~/Documents/Current_Projects/Sexual_Assault/new_SA_YNHH/Updated YNHH SA code/C_F_AllPatients_2020to2023.xlsx") %>% select(1, contains("Location")) %>% rename("pat_enc_csn_id" = "Pat Enc Csn ID...1") %>% clean_names()
...
all_pts <- all_pts %>% mutate(
  ...
  sane_kit_offered = case_when(
    ...
    sane_kit_done == 0 | sane_kit_done == 3 | sane_kit_done == 4 ~ "Sane Kit Not Offered"
  ),
  sane_ED = if_else(
    discharge_location %in% c("YSC", "YSC-CIU", "SRC"), 1,0 
    ),
  pathway = if_else(date(arrive_dt) < date(pathway_start), "Didn't Use Pathway", pathway),
  ...
  sane_advocate_offered = if_else(advocate_offered == "Advocate Offered" & sane_kit_offered == "Sane Kit Offered", 1, 0),
...
number_of_minors <-all_pts %>% filter(age < 18) #changed it so age is first exclusion criteria
...
excluded_patients <- pp.cohort.1  %>% mutate(exclude = ifelse(sane_kit_done == 4, NA, exclude)) %>% filter(!is.na(exclude)) %>%
  mutate(reason_to_exclude = if_else(sane_kit_done == 4, 3, reason_to_exclude)) #if sane kit not done b/c psych that is also reason to eclude now
between_pts <- all_pts %>% filter(age > 17, between == 1)
...
pp.cohort.2$exposure.tab <- factor(pp.cohort.2$exposure.tab, levels=c("Pre-intervention", "Post-intervention"))
pp.table.1 <- tbl.one.fxn(pp.cohort.2) %>% add_overall()
...
pp.cohort.2$exposure.tab <- factor(pp.cohort.2$exposure.tab, levels=c("Pre-intervention", "Post-intervention"))
pp.table.2 <-tbl.two.fxn(pp.cohort.2) %>% add_overall()
...
sane.off.reg<-function(data, exposure){
  data <- data %>% filter(sane_kit_done != 4)
  model <-  glm(sane_kit_offered ~ exposure + age + white + ipv + intoxicated + sane_ED, family = "binomial", data = data)
  model.results <- tbl_regression(model, exponentiate = TRUE) %>% add_global_p()
  return(model.results)
}
```
...
pp.u120.sk_o <- pp.cohort.2 %>%  filter(under120h == TRUE, sane_kit_offered == "Sane Kit Offered", female == 1, english == 1)
...
excluded_plot <- all_excluded %>% filter(between == 0) %>%  ggplot(aes(x = reason_to_exclude)) + geom_bar(fill = "darkred") + geom_text(stat='count', aes(label=..count..), vjust=-1) + theme_minimal()
all_excluded

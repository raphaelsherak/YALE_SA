...
advocate.regression<-function(x){
  x$advocate_offered <- as_factor(x$advocate_offered )
  x$advocate_offered <- relevel(x$advocate_offered, ref= "No Documentation of Pt Advocate")
  x <- x %>% mutate(exposure = if_else(pathway == "Used Pathway", 1, 0))
  model <-  glm(advocate_offered ~ exposure + age + female + white + english + ipv, family = "binomial", data = x)
